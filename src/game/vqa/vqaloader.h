...
struct VQAHeader; // This forward declares the header struct so you can have types that have pointers to it.
struct VQAHandle;
struct VQAConfig;
struct VQACBNode;
struct VQAFrameNode;
...
typedef void(__cdecl *UnVQFuncPtr)(uint8_t *, uint8_t *, uint8_t *, unsigned, unsigned, unsigned);
...
#endif // SYSTEM_LITTLE_ENDIAN || SYSTEM_BIG_ENDIAN
...
#define CHUNK_NAME \
    MAKE_CHUNK('N', 'A', 'M', 'E') // Standard IFF Name chunk used in Poly,BR and LOL game VQA's, poly contains the string
...
#define CHUNK_CAP0 MAKE_CHUNK('C', 'A', 'P', '0') // Isn't in any VQA's, present in early TD code, captions(subtitles) where in there compressed with LCW
...
DEFINE_ENUMERATION_BITWISE_OPERATORS(VQADataFlagEnum);
...
struct VQAData
{
    ...
    UnVQFuncPtr UnVQ;
    VQAFrameNode *m_FrameData;
    VQACBNode *m_CBData;
    VQAAudio m_Audio;
    VQALoader m_Loader;
    VQAChunkHeader m_Chunk;
    VQADrawer m_Drawer;
    VQAFlipper m_Flipper;
    unsigned m_Flags; // VQADataFlagEnum
    int *m_Foff;
    int m_VBIBit;
    int m_MaxCBSize;
    int m_MaxPalSize;
